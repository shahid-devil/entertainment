...
MyPnky.addCommand({pattern: 'break ?(.*)', fromMe: false, dontAddCommandList: true}, (async (message, match) => {
 ...
 MyPnky.addCommand({pattern: 'emo ?(.*)', fromMe: false, desc: 'emogi to png'}, (async (message, match) => {
    ...
    MyPnky.addCommand({pattern: 'wanted ?(.*)', fromMe: false, dontAddCommandList: true}, (async (message, match) => {

    if (match[1] === '') return await message.client.sendMessage(message.jid,NEED_WORD);

    var webimage = await axios.get(`https://videfikri.com/api/textmaker/wanted/?text=${match[1]}`, { responseType: 'arraybuffer' })

    await message.client.sendMessage(message.jid,Buffer.from(webimage.data), MessageType.image, {mimetype: Mimetype.jpg, caption: '```𝙼𝚊𝚍𝚎 𝚋𝚢 𝚂𝚑𝚊𝚑𝚒𝚍 𝚂𝚎𝚛😈```'})

    }));
    
    MyPnky.addCommand({ pattern: '2ttp ?(.*)', fromMe: false, dontAddCommandList: true }, (async (message, match) => {
    ...
    MyPnky.addCommand({ pattern: '2attp ?(.*)', fromMe: false, dontAddCommandList: true }, (async (message, match) => {
